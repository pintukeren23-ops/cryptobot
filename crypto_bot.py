...
async def get_candle_data(network, pool_address, timeframe="hour", aggregate=1, limit=48):
    ...
    if not data:
        return None
    try:
        # Tiap item: [timestamp, open, high, low, close, volume]
        return data["data"]["attributes"]["ohlcv_list"]
    except (KeyError, TypeError):
        return None

def hitung_rsi(closes, periode=14):
    """Hitung RSI dari list harga close (urutan lama ke baru)."""
...
def cari_support_resistance(highs, lows):
    """Cari level support & resistance sederhana dari swing low/high."""
    if not highs or not lows:
        return None, None
    return min(lows), max(highs)

def hitung_indikator(ohlcv_list):
    """Olah raw candle GeckoTerminal jadi dict indikator siap pakai."""
    ...
    rsi = hitung_rsi(closes, periode=min(14, len(closes) - 1))
    sma20 = hitung_sma(closes, periode=20)
    support, resistance = cari_support_resistance(highs, lows)
...
def ai_analisis(token_info, harga, indikator=None, rug_data=None, holder_info=None):
    try:
        ...
        if indikator and indikator.get("support") and indikator.get("resistance"):
            indikator_text = (
                "\nDATA TEKNIKAL (dihitung dari candle historis riil, " + str(indikator["jumlah_candle"]) + " candle terakhir):\n"
                "RSI(14): " + (str(indikator["rsi"]) if indikator["rsi"] is not None else "N/A") + " (>70 overbought, <30 oversold)\n"
                "SMA20: $" + str(round(indikator["sma20"], 10)) + "\n"
                "Support (low terendah): $" + str(round(indikator["support"], 10)) + "\n"
                "Resistance (high tertinggi): $" + str(round(indikator["resistance"], 10)) + "\n\n"
                "PENTING: Gunakan level Support dan Resistance di atas sebagai acuan utama untuk TP dan entry. "
                "Target profit (tp1_harga/tp2_harga) sebaiknya berada di sekitar atau sedikit di atas level Resistance, "
                "bukan angka sembarang. Entry ideal sebaiknya mendekati level Support. "
                "Jangan berikan angka yang jauh di luar range Support-Resistance ini kecuali ada alasan kuat dari data volume/momentum.\n"
            )
        else:
            indikator_text = (
                "\nCATATAN: Data candle historis token ini belum cukup (token kemungkinan masih sangat baru). "
                "Tidak ada data teknikal (RSI/Support/Resistance) yang bisa dihitung. "
                "Beri peringatan eksplisit di bagian alasan bahwa prediksi sangat tidak pasti karena minim histori harga.\n"
            )
        ...
        if rug_data:
            ...
            keamanan_text += "\nSKOR KEAMANAN RUGCHECK (data riil, bukan tebakan): " + str(skor_rugcheck) + "\n"
        if holder_info:
            keamanan_text += (
                "Top 1 holder pegang: " + str(holder_info["holder_terbesar_pct"]) + "% supply\n"
                "Top 10 holder pegang: " + str(holder_info["top10_pct"]) + "% supply\n"
                "(Top 10 holder > 50% supply = risiko whale dump tinggi)\n"
            )
        if keamanan_text:
            keamanan_text += "WAJIB dasarkan status_keamanan pada data RugCheck/holder di atas, bukan asumsi sendiri.\n"
        else:
            keamanan_text = "\nCATATAN: Data RugCheck tidak tersedia untuk token ini, nilai status_keamanan dengan hati-hati dan sebut di alasan bahwa belum ada verifikasi keamanan otomatis.\n"

        prompt = (
            "Kamu adalah analis trading meme coin profesional dan berpengalaman. Analisis data token berikut secara mendalam.\n\n"
            "Data Token:\n" + token_info + "\n"
            "Harga token saat ini: $" + str(harga) + "\n"
            + indikator_text + keamanan_text + "\n"
            "Tugasmu: Berdasarkan data di atas, prediksi seberapa jauh harga token ini bisa naik atau turun. "
            "Tentukan harga entry ideal, harga target profit (TP), dan harga stop loss (SL) secara realistis.\n\n"
            ...
            '  "entry_pct": -5,\n'
            '  "entry_note": "1 kalimat alasan entry di level itu",\n'
            '  "tp1_harga": 0.00025,\n'
            '  "tp1_alasan": "1 kalimat kenapa harga ini jadi target",\n'
            '  "tp2_harga": 0.00035,\n'
            '  "tp2_alasan": "1 kalimat kenapa harga ini jadi target (kosongkan/0 jika tidak ada)",\n'
            '  "sl_pct": 15,\n'
            '  "alasan": "2-3 kalimat analisis keseluruhan"\n'
            "}\n\n"
            "Keterangan:\n"
            "- entry_pct: angka negatif = tunggu harga turun dulu, 0 = bisa entry sekarang\n"
            "- tp1_harga & tp2_harga: prediksi harga dalam USD. Jika ada data teknikal, dasarkan pada level Support/Resistance yang diberikan\n"
            "- sl_pct: angka positif (misal 15 = cut loss jika turun 15%)\n"
        ...
        # Sanity check: kalau ada data resistance riil, cap TP yang kelewat liar
        if indikator and indikator.get("resistance"):
            ...
            if tp2_harga > batas_atas:
                ...
                tp2_alasan = "(disesuaikan ke atas level resistance historis)"

        # Hitung persentase TP dari harga entry untuk info tambahan
        ...
        if tp1_harga:
            hasil += "  TP1: " + format_harga(tp1_harga) + " (+" + str(int(tp1_pct)) + "% dari sekarang)\n"
            if tp1_alasan:
                hasil += "  " + tp1_alasan + "\n"
        if tp2_harga:
            hasil += "\n  TP2: " + format_harga(tp2_harga) + " (+" + str(int(tp2_pct)) + "% dari sekarang)\n"
            if tp2_alasan:
                hasil += "  " + tp2_alasan + "\n"
        hasil += (
        ...
        # Data mentah dikembalikan juga buat disimpan ke history (fitur tracking akurasi)
        raw_data = {
...
async def analisis_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if chain and pool_address:
            ohlcv_list = await get_candle_data(chain, pool_address, timeframe="hour", aggregate=1, limit=48)
            indikator = hitung_indikator(ohlcv_list)
        ...
        ai_result, raw_data = ai_analisis(token_info, harga, indikator, rug_data, holder_info)

        # Simpan ke riwayat buat tracking akurasi (fitur #1), kalau parsing AI berhasil
        if raw_data:
        ...
        if indikator:
            teknikal_text = (
                "DATA TEKNIKAL (" + str(indikator["jumlah_candle"]) + " candle 1H terakhir):\n"
                "RSI(14): " + (str(indikator["rsi"]) if indikator["rsi"] is not None else "N/A") + "\n"
                "Support: $" + format_harga(indikator["support"])[1:] + "\n"
                "Resistance: $" + format_harga(indikator["resistance"])[1:] + "\n\n"
            )
        else:
            teknikal_text = "DATA TEKNIKAL: Belum tersedia (token terlalu baru, histori candle minim)\n\n"
